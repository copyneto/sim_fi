*&---------------------------------------------------------------------*
*& Report ZRACORR_RENUM_SUBTA_ANEK_KEY_3
*&---------------------------------------------------------------------*
...
p_anln1 TYPE anln1 OBLIGATORY,
p_anln2 TYPE anln2 OBLIGATORY DEFAULT '0000',
...
DATA:
      ...
      lt_items_repair_rev     TYPE TABLE OF ls_ty_lineitems,
...
DATA(mo_asset) = cl_faa_mdo_factory_static=>get_instance_root(
      ...
      iv_asset_no     = p_anln1
      iv_asset_subno  = p_anln2
      iv_mode         = if_faa_frw_constants=>gc_mode-UPDATE ).

SORT lt_items_for_repair BY subta awtyp awref ldgrp afabe.
  ...
  OR lv_last_awref <> <ls_item_repair>-awref.
    ...
    IF <ls_item_repair>-subta_rev IS NOT INITIAL.
      ...
      IF lt_items_repair_rev_doc IS NOT INITIAL.
        APPEND LINES OF lt_items_repair_rev_doc TO lt_items_repair_rev.
      ENDIF.
    ENDIF.
  ENDIF.
...
IF p_test IS INITIAL.
  LOOP AT lt_items_for_repair ASSIGNING <ls_item_repair>.
  ...
  IF lt_items_repair_rev IS NOT INITIAL.
    LOOP AT lt_items_repair_rev ASSIGNING <ls_item_repair>.
      IF <ls_item_repair>-statistic_item = abap_false.
...
ls_color_old-COLOR-col = COL_NEGATIVE.
LOOP AT lt_items_repair_rev ASSIGNING <ls_item_repair_rev>.
  APPEND ls_color_new TO <ls_item_repair_rev>-COLOR.
  APPEND ls_color_old TO <ls_item_repair_rev>-COLOR.
ENDLOOP.

APPEND LINES OF lt_items_for_repair TO lt_output.
APPEND LINES OF lt_items_repair_rev TO lt_output.
